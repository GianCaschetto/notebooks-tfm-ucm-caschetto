# Notebooks del TFM (provisional)

Repositorio de notebooks de análisis y modelado asociados al trabajo de fin de máster sobre **Faster Order**: datos de pedidos, demanda por producto e ingresos por periodo, en línea con el resto del pipeline (features, modelos, explicabilidad).

**Estado:** README provisional; solo se documentan aquí, con detalle, los tres primeros notebooks. El resto (`03_` … `08_`) comparte la misma carpeta `data/` y el orden lógico del TFM.

## Requisitos generales

- Python 3.x con `pandas`, `numpy`, `matplotlib`, `seaborn`, `holidays`, `scipy` (y en notebooks posteriores las librerías que uses para modelado y SHAP).
...
- **Para qué sirve:** Entender patrones de demanda y dejar la tabla base para el *feature engineering* de demanda (`03_…`).
...
- **Para qué sirve:** Caracterizar la evolución del negocio en dinero y volumen diario y alimentar el notebook de features de ingresos (`04_…`).
- **Datos:** Misma carpeta `../data/`; ajusta el nombre del fichero de pedidos si no es el export `triplekb-orders-…`.

---

*Última actualización: README provisional; ampliar con `03_`–`08_` cuando cierres el pipeline.*
